...
## 🛠️ 2026-06-14 系統優化更新 (移除預設假資料與 UX 優化)

為了提升儀表板的專業度與「即時監控」真實感，我們進行了以下優化：

1. **資料庫清空 (清除歷史預設資料)**：
   * 執行了 `TRUNCATE "Detections"` 清空資料庫中先前殘留的測試紀錄（如非當前偵測的 `person`、`bus` 等），避免初次載入時於「即時偵測日誌」與圖表中顯示過往預設資料。
   * 現在儀表板在重整或剛啟動時，只會顯示本期即時偵測到的資料。

2. **影像監控預設佔位畫面優化**：
   * 移除了未啟動偵測時預載的 YOLO 測試用 `bus.jpg` 靜態圖，改為高質感的深色**「即時影像辨識未啟動」佔位區**。
   * 配合系統一貫的 Glassmorphism 暗色科技風，加入專屬的相機停用圖示（`fa-video-slash`）與提示字樣，使整體介面質感更上一層樓。

3. **「截圖存檔」按鈕防呆機制**：
   * 當影像辨識未啟動（沒有視訊串流與偵測框）時，將「截圖存檔」按鈕設為 **`disabled` 狀態**，防止使用者下載到空白或無效的測試圖。
   * 啟動即時辨識後會自動啟用按鈕；關閉辨識時會再度停用，大幅增強了互動設計的防呆與邏輯性。
