...
def main():
    ...
    add_bullet_points(slide, start_left + col_width + gap + Inches(0.2), top + Inches(0.3), col_width - Inches(0.4), height - Inches(0.6), [
        "• 即時警報發送：偵測到安全違規行為時，自動調用 Webhook，將警告文字訊息即時發送至 Discord 頻道。",
        "• 違規畫面截圖：自動擷取當下違規畫面的圖片作為附件一併發送，提供管理人員明確的第一手事證。",
        "• 去抖動與冷卻機制：內建 10 秒警報冷卻時間，避免在連續畫面中重複發送警報而洗版。"
    ], title="🚨 Discord 即時警報", title_size=18, text_size=13)
    ...
    add_bullet_points(slide, left + Inches(0.2), start_top + (row_height + row_gap)*2 + Inches(0.15), row_width - Inches(0.4), row_height - Inches(0.2), [
        ...
        "• Discord Webhook 警報：負責向外部 Discord 頻道發送包含即時違規影像截圖的訊息通知，落實主動警報。"
    ...
    steps_data = [
        ...
        {"num": "4", "title": "Discord Webhook", "desc": ["• 自動呼叫 Webhook 將警報文字及違規照片傳送至 Discord", "• 內建 10 秒去抖動冷卻時間，防止洗版"]}
    ]
    ...
    add_bullet_points(slide, Inches(1.1), Inches(1.9), Inches(5.0), Inches(4.4), [
        ...
        "• 工安管理數位化：結合資料庫與 AI 分析，將冰冷的監控影像轉化為具備管理價值的工安稽核報告。",
    ...
    # ----------------------------------------------------
    slide = prs.slides.add_slide(blank_layout)
    apply_background(slide, RGBColor(15, 23, 42))
